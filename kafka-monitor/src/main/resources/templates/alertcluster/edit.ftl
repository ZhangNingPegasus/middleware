        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline" style="width:700px">
                <select name="type" lay-filter="type" lay-verify="required" lay-search>
                    ...
                    <#list type as t>
                        <option value="${t.code}" ${(item.type==t.code)?string('selected=selected','')}>${t.description}</option>
                    </#list>
                </select>
            </div>
        </div>
            ...
            <div class="layui-input-inline" style="width:700px">
                <select name="server" lay-filter="server" lay-verify="required" lay-search>
                    ...
                    <#list servers as server>
                        <option value="${server}" ${(item.server==server)? string('selected=selected','')}>${server}</option>
                    </#list>
                </select>
            </div>
        </div>
    ...
    <script>
        layui.config({base: '../../..${ctx}/layuiadmin/'}).extend({index: 'lib/index'}).use(['index', 'form'], function () {
            ...
            form.on('select(type)', function (data) {
                ...
                admin.post('listServers', {'type': data.value, 'opt': 'update'}, function (res) {
