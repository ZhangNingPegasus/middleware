 ...
 */
@Controller
@RequestMapping(ClusterController.PREFIX)
public class ClusterController {
    public static final String PREFIX = "cluster";
    private final KafkaZkService kafkaZkService;
    private final KafkaService kafkaService;

    public ClusterController(final KafkaZkService kafkaZkService,
    ...
    public String tosetting(final Model model) throws Exception {
        final String kafkaUrl = this.kafkaService.getBootstrapServers();
        final String springboot = String.format("spring:\n" +
                "  kafka:\n" +
                "    #kafka集群地址，多个用逗号隔开\n" +
                "    bootstrap-servers: %s\n" +
                "    \n" +
        ...
        final String java = String.format("package com.xxx.xxx.xxx;\n" +
                "\n" +
    ...
    public Result<TreeInfo> getChartData() throws Exception {
        final TreeInfo root = new TreeInfo("Kafka集群");
        final List<KafkaBrokerVo> allBrokers = this.kafkaService.listBrokerInfos();
        final List<TreeInfo> children = new ArrayList<>(allBrokers.size());
        for (final KafkaBrokerVo broker : allBrokers) {
            final TreeInfo treeInfo = new TreeInfo(String.format("[%s] : %s", broker.getName(), broker.getHost()));
            treeInfo.setStyle(Style.success());
